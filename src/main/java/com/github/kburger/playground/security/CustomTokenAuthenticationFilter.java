...
public class CustomTokenAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        if (whitelist.contains(request.getServletPath())) {
            logger.info("whitelisted path {}, authenticating as anonymous", request.getServletPath());
            return new CustomToken("anon");
        }
        
        Optional<String> token = Optional.ofNullable(request.getHeader(HEADER_TOKEN));
        
        if (!token.isPresent()) {
            throw new AuthenticationServiceException("missing token");
        }
        
        return getAuthenticationManager().authenticate(new CustomToken(token.get()));
    }
}
